...
from .heuristics import AnalysisInputError, ReachabilityError, extract_url_features, get_domain_intelligence, normalize_url, sanitized_domain, url_hash, validate_url
...
def _build_session() -> requests.Session:
    ...
    retry = Retry(total=1, backoff_factor=0.2, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=['GET', 'HEAD'])
...
def fetch_page(url: str, timeout: int, max_redirect_depth: int, retry_count: int) -> PageFetchResult:
    ...
    for _attempt in range(retry_count + 1):
        try:
            response = session.get(url, timeout=timeout, allow_redirects=True, headers={'User-Agent': 'Detector/1.0'})
...
def analyze_page(url: str, timeout: int, max_redirect_depth: int, retry_count: int) -> tuple[dict[str, float], PageFetchResult]:
...
def filtered_reports(*, page: int, per_page: int, label: str | None, domain: str | None, date_from: str | None, date_to: str | None):
...
def label_counts_by_day(limit_days: int = 7) -> list[dict[str, Any]]:
    ...
    for day in list(sorted(buckets))[-limit_days:]:
        counter = buckets[day]
        rows.append({'day': day, 'safe': counter.get('safe', 0), 'suspicious': counter.get('suspicious', 0), 'phishing': counter.get('phishing', 0)})
    return rows
...
def analyses_to_csv(rows: list[Analysis]) -> str:
    ...
    for row in rows:
        writer.writerow([row.id, row.normalized_url, row.domain, row.risk_score, row.label, row.reachability, row.created_at.isoformat()])
