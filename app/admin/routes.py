...
from flask import Blueprint, Response, current_app, flash, redirect, render_template, send_file, request, url_for
...
from app.phishing.services import analyses_to_csv, filtered_reports, label_counts_by_day, recent_analyses, run_analysis, serialize_analysis, top_phishing_domains
...
def batch_analyze():
    ...
    for row in rows:
        writer.writerow([row.get('analysis_id', ''), row.get('url', ''), row.get('risk_score', ''), row.get('label', ''), '; '.join(row.get('reasons', []))])
    db.session.add(Report(title='Batch CSV analysis', content=f'Processed {len(rows)} URLs'))
    db.session.commit()
    return Response(output.getvalue(), mimetype='text/csv', headers={'Content-Disposition': 'attachment; filename=batch-analysis-results.csv'})
...
def export_csv():
    ...
    db.session.commit()
    return Response(analyses_to_csv(rows), mimetype='text/csv', headers={'Content-Disposition': 'attachment; filename=detector-report.csv'})
